...
async function renderTelaQuiz() {
  ...
  const [{ data: quizzes }, { data: ranking }] = await Promise.all([
    ...
    supabaseClient.from("profiles")
      .select("id, username, quiz_nivel, quiz_xp, quiz_acertos_total, verificado, foto_url")
    ...
    <div class="card" id="ranking-lista">
      ${(ranking || []).map((u, i) => `
        <div class="ranking-item">
          ...
          <div class="ranking-avatar">${avatarHtml(u.foto_url, u.username)}</div>
          <div class="ranking-info">
            <div class="ranking-nome">@${escapeHtml(u.username)}${badgeVerificado(u.verificado)}</div>
